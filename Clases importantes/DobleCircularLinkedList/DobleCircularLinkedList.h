...
using namespace vcn;
    ...
    template <class T>
    class DobleCircularLinkedList :  public LinkedList<T> {
    ...
    public:
        DobleCircularLinkedList() {}
        virtual ~DobleCircularLinkedList();
        ...
        friend std::ostream & operator <<(std::ostream &, DobleCircularLinkedList<Tn> &);

    };

    template <class T>
    DobleCircularLinkedList<T>::~DobleCircularLinkedList()
    {
        this->clear();
    }

    template <class T>
    Node<T> * DobleCircularLinkedList<T>::last()
    {
        return this->_last;
    }

    template  <class T>
    void DobleCircularLinkedList<T>::insert(T element, int position)
    {
    ...
    template  <class T>
    void DobleCircularLinkedList<T>::insert(Node<T> * newnode, int position)
    {
    ...
    template  <class T>
    Node<T> * DobleCircularLinkedList<T>::remove(int position)
    {
        ...
        if (position == 0) {
            ...
            this->_first->setPrevious(this->_last); //N
            this->_last->setNext(this->_first);
    ...
    template  <class T>
    Node<T> *  DobleCircularLinkedList<T>::at(int position)
    {
        /* Cuando la lista está vacía o position es inválida */
        if (this->empty() || (position < 0 || position >= this->_size )) {
            return nullptr;
    ...
    template  <class T>
    void DobleCircularLinkedList<T>::clear()
    {
        /* Cuando la lista está vacía */
    ...
    template <class T>
    std::ostream & operator <<(std::ostream & os, DobleCircularLinkedList<T> & list)
    {
        for (auto node : list )
        {
            os << node;
